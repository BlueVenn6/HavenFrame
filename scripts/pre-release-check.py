...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run pre-release checks for the bilingual HavenFrame workspace.")
    ...
    if not args.skip_heavy:
        ...
        checks.append(_command_check("mobile_typecheck", [npm, "run", "typecheck"], root / "mobile-expo"))
        checks.append(_command_check("mobile_i18n_test", [npm, "run", "test:i18n"], root / "mobile-expo"))
        checks.append(_command_check("mobile_platform_parity", [npm, "run", "test:platform-parity"], root / "mobile-expo"))
        checks.append(_command_check("mobile_runtime_selection", [npm, "run", "test:runtime-selection"], root / "mobile-expo"))
        checks.append(_command_check("mobile_report", [npm, "run", "test:report"], root / "mobile-expo"))
        checks.append(_command_check("mobile_android_internal_release", [npm, "run", "build:android:internal"], root / "mobile-expo"))
        checks.append(_command_check("ios_shared_bundle_local", [npm, "run", "bundle:ios:local"], root / "mobile-expo"))
    ...
    checks.append(
        _file_contains_check(
            ...
            "third_party_notices",
            ["Third-Party Notices", "THIRD_PARTY_LICENSES/EXPO-MIT.txt"],
        )
    )
    checks.append(
        _file_contains_check(
            root / "THIRD_PARTY_LICENSES" / "EXPO-MIT.txt",
            "expo_license_notice",
            ["MIT License", "650 Industries, Inc."],
        )
    )
    checks.append(
        _file_contains_check(
    ...
    checks.append(_file_contains_check(root / "SECURITY.md", "security_doc", ["本机后端", "API Key", "中转", "127.0.0.1:8010"]))
    checks.append(_file_contains_check(root / "README.md", "readme_security_section", ["## 安全与发布", "API Key", "workspace/", "com.havenframe.desktop"]))
    checks.append(
        _file_contains_check(
    ...
    for document in ("CODEBASE_AUDIT.md", "ARCHITECTURE.md", "PLATFORM_FEATURE_MATRIX.md", "API_PROVIDER_MATRIX.md", "TEST_REPORT.md", "CLEANUP_REPORT.md", "RELEASE_READINESS.md"):
        checks.append(_file_contains_check(root / document, f"release_doc_{document.lower()}", ["栖构"]))
    ...
    checks.append(_file_contains_check(root / "mobile-expo/scripts/test-platform-parity.mjs", "mobile_desktop_capability_absent", ["mobile must not ship a desktop capability registry", "/api/local", "/api/render-engines"]))
    checks.append(_file_contains_check(root / "mobile-expo/plugins/with-local-android-build.js", "android_utf8_local_build", ["Dfile.encoding=UTF-8", "org.gradle.parallel"]))
    checks.append(_cloud_build_policy_check(root))
...
def _cloud_build_policy_check(root: Path) -> tuple[str, bool, str]:
    package_path = root / "mobile-expo" / "package.json"
    script_path = root / "scripts" / "pre-release-check.py"
    package_text = package_path.read_text(encoding="utf-8", errors="ignore") if package_path.exists() else ""
    script_text = script_path.read_text(encoding="utf-8", errors="ignore") if script_path.exists() else ""
    text = f"{package_text}\n{script_text}"
    if re.search(r"\beas\s+build\b", text, flags=re.IGNORECASE):
        return "no_cloud_build_trigger", False, "cloud build CLI invocation detected"
    return "no_cloud_build_trigger", True, "Windows/Android local only; iOS validation is a local JS bundle, not a cloud build"
...
def _version_consistency_check(root: Path) -> tuple[str, bool, str]:
    ...
    try:
        ...
        mobile_package = json.loads(required[4].read_text(encoding="utf-8"))
        mobile_config = json.loads(required[5].read_text(encoding="utf-8"))["expo"]
    ...
    mobile_version = str(mobile_package["version"])
    desktop_ok = (
    ...
    mobile_ok = mobile_config.get("version") == mobile_version
    if not desktop_ok or not mobile_ok:
        return "release_version_consistency", False, "desktop or mobile release metadata is inconsistent"
    return "release_version_consistency", True, f"desktop {desktop_version}; mobile {mobile_version}"
