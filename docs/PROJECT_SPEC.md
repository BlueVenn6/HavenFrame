...
本公开仓库只维护 Windows 桌面版的中英双语功能，不包含 Android/iOS 客户端源码。切换语言不得改变模型、Provider、项目数据或任务行为。
...
桌面版保留内嵌 FastAPI、SQLite、workspace 与任务队列作为应用基础设施，但不提供本地部署页面、本地模型服务或本地 renderer。
